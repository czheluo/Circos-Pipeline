...
#$cnv=ABSOLUTE_DIR($cnv);
#$sv=ABSOLUTE_DIR($sv);
#$snpplusindel=ABSOLUTE_DIR($snpplusindel);
#$ssr=ABSOLUTE_DIR($ssr);
#$paralist=ABSOLUTE_DIR($paralist);
...
while (<In>) {
	...
	if($para[4] eq "indel" && $para[5] ne "NA"){######plot indel
		my $nsam;
		my @sample;
		my $max;    
		my %hash;
	...
	if ($para[3] eq "snpplusindel" && $para[5] ne "NA"){######plot $snpplusindel
		my $nsam;
		my @sample;
		my $max;
		my %hash;
	...
	}elsif($para[4] eq "snpplusindel" && $para[5] eq "NA"){
		open In,$snpplusindel;
		if ($snpplusindel=~/.gz$/) {
			close In;
			open In,"gunzip -c $snpplusindel|";
	...
	if($para[3] eq "cnv" && $para[5] ne "NA"){######plot cnv both same format
		open In,$cnv;
		my $max;    
		my %hash;
		my $win=$para[6];
	...
	if($para[3] eq "sv" && $para[5] ne "NA"){######plot sv
		my $nsam;
		my @sample;
		my $max;
		my %hash;
	...
	if($para[3] eq "gene" && $para[5] ne "NA"){
		open In,$gff;
	#print $gff;die;
		my $max;    
		my %hash;
...
<<include $outdir/draw.circos/housekeeping.conf>>
	...
	if ($para[1] == 1){##forth color type
		...
		close Color;
	}elsif($para[1] == 2){##second color type
		...
		print Color "bandcol10=rgb(105,105,105)\n";
		close Color;
	}elsif($para[1] == 3) {##first color type
		...
		print Color "bandcol10=rgb(105,105,105)\n";
		close Color;
}elsif($para[1] ==4){##third color type
