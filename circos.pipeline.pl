...
while (<PA>) {
	...
	}elsif($para[4] eq "indel" && $para[5] ne "NA"){######plot indel
		my $nsam;
		my @sample;
		my $max;    
		my %hash;
    ...
    #print $max;
	...
	}elsif($para[3] eq "ssr" && defined $para[5]){######plot ssr
		open In,$ssr;
		my $max;    
		my %hash;
		my $win=$para[6];
		...
		open In,$gff or die "Can't open '$gene': $!";
		#print $gff;die;
		my $max;    
		my %hash;
		my $win=$para[6];
...
#print $para[1];die;
if($para[1] eq "1"){##forth color type
	...
	print Color "bandcol10=rgb(105,105,105)\n";
	close Color;
}elsif($para[1] eq "3") {##first color type
	...
	print Color "bandcol10=rgb(105,105,105)\n";
	close Color;
}elsif($para[1] eq "4"){##third color type
